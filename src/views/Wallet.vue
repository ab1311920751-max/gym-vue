<template>
  <div>
         ...
         <el-card shadow="hover">
            ...
            <div style="display: flex; flex-direction: column; gap: 15px;">
               <div style="display: flex; align-items: center; gap: 10px;">
                   ...
                   <el-tag v-if="user.vipType === VIP_TYPE.NORMAL" type="info">普通会员</el-tag>
                   <el-tag v-else-if="user.vipType === VIP_TYPE.MONTHLY" type="primary">月卡 VIP</el-tag>
                   <el-tag v-else-if="user.vipType === VIP_TYPE.YEARLY" type="warning">👑 年卡 VIP</el-tag>
               </div>
    ...
    <div style="margin-top: 30px">
        ...
        <el-row :gutter="20">
            <el-col :span="12">
                <el-card shadow="hover" class="vip-card" :class="{ 'active': user.vipType === VIP_TYPE.MONTHLY }">
                    <div style="text-align: center; padding: 20px;">
                        ...
                        <el-button type="primary" plain @click="handleBuyVip(VIP_TYPE.MONTHLY)">
            ...
            <el-col :span="12">
                <el-card shadow="hover" class="vip-card" :class="{ 'active': user.vipType === VIP_TYPE.YEARLY }" style="border: 1px solid #e6a23c;">
                    <div style="text-align: center; padding: 20px;">
                        ...
                        <el-button type="warning" effect="dark" @click="handleBuyVip(VIP_TYPE.YEARLY)">
...
const loadUser = async () => {
    ...
    try {
        const res = await userApi.detail(localUser.id)
        if (res.code === '200') {
            user.value = res.data
...
const checkPayCallback = async () => {
    if (route.query.pay === 'success') {
        ...
        try {
            const res = await userApi.recharge({ userId: user.value.id, amount: Number(total_amount) })
            if (res.code === '200') {
...
const handleBuyVip = (type) => {
    const price = type === VIP_TYPE.MONTHLY ? 30 : 300
    ...
    .then(async () => {
        await userApi.buyVip({ userId: user.value.id, vipType: type })
